...
export function DashboardLayout({ children, activeView, username, isPro, isSessionActive, onNavigate, onLogout }: DashboardLayoutProps) {
    ...
    return (
        <div className="flex h-screen bg-black text-white overflow-hidden font-[family-name:var(--font-geist-sans)]">
            ...
            {/* Sidebar */}
            <aside className={`fixed lg:static inset-y-0 left-0 w-64 bg-zinc-950 border-r border-white/10 flex flex-col p-6 z-40 transition-transform duration-300 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
            ...
            <main className="flex-1 overflow-auto bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-zinc-900 via-black to-black relative flex flex-col">
                ...
                <header className="relative z-20 flex justify-between lg:justify-end items-center gap-4 p-4 lg:p-6">
                    {/* Mobile Menu Toggle */}
                    <button
                    ...
                    <div className="flex items-center gap-3">
                        {isSessionActive && (
                            <div className="hidden sm:flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-full animate-in fade-in slide-in-from-right-4">
                        ...
                        <div
                            ...
                            className="flex items-center gap-2 lg:gap-3 bg-white/5 border border-white/10 px-3 py-1.5 lg:px-4 lg:py-2 rounded-2xl backdrop-blur-md cursor-pointer hover:bg-white/10 transition-all group"
                        >
                            <div className="w-1.5 h-1.5 lg:w-2 lg:h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]"></div>
                            <div className="flex flex-col">
                                <span className="text-[8px] lg:text-[10px] text-white/40 uppercase font-bold tracking-wider leading-none mb-1">Smart Wallet</span>
                                <code className="text-[10px] lg:text-xs text-emerald-400 font-mono leading-none">
                                    {smartWalletPubkey?.toBase58().slice(0, 4)}...{smartWalletPubkey?.toBase58().slice(-4)}
                                </code>
                            </div>
                            <Copy size={12} className="text-white/20 group-hover:text-white/60 transition-colors" />
                        </div>
                    </div>
                </header>

                <div className="flex-1 p-4 lg:p-8 pt-2 max-w-6xl mx-auto w-full relative z-10">
                    {children}
                </div>
            </main>
        </div>
